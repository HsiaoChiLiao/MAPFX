...
## Using the Example Raw Files in MAPFX Package for this Vignette
### MPC
This dataset is a subset of the single-cell murine lung data at steady state downloaded from [FlowRepository](https://flowrepository.org/id/FR-FCM-Z2LP) provided by Etienne Becht (Nov 2020). The raw data contains 266 .FCS files from 266 wells with 50 cells in each file.

### FFC
This dataset contains 50 cells (sorted CD4+ and CD8+ T cells) in each .FCS files from mice splenocytes that was down-sampled from [FlowRepository](http://flowrepository.org/id/FR-FCM-Z6UG) provided by Jalal Alshaweesh (Oct 2023). Batch information is in the `filename_meta.csv` file in the `inst/extdata/FFC/meta/` directory.
...
dir.create(file.path(getwd(), "MPC_impu_Output"))
...
MapfxMPC(
    ...
    Outpath = file.path(getwd(), "MPC_impu_Output"), # or users specify their own path: "/Outpath/Output/",
    file_meta = "auto",
    bkb.v = c(
...
# create an Output directory in the current working directory for the argument 'Outpath' of the MapfxMPC function
dir.create(file.path(getwd(), "MPC_NOimpu_Output"))
...
MapfxMPC(
    ...
    runVignette_rawInten = ord.fcs.raw.mt_mpc, #set NULL if not running this Vignette
    FCSpath = NULL, # users specify their own path: "/FCSpath/FCS/",
    Outpath = file.path(getwd(), "MPC_NOimpu_Output"), # or users specify their own path: "/Outpath/Output/",
    file_meta="auto",
    bkb.v = c(
    "FSC-H", "FSC-W", "SSC-H", "SSC-W", "CD69-CD301b", "MHCII", 
    "CD4", "CD44", "CD8", "CD11c", "CD11b", "F480", 
...
# create an Output directory in the current working directory for the argument 'Outpath' of the MapfxMPC function
dir.create(file.path(getwd(), "FFCnorm_Output"))
...
MapfxFFC(
    ...
    FCSpath = NULL, # users specify their own path: "/FCSpath/FCS/",
    Outpath = file.path(getwd(), "FFCnorm_Output"), # or users specify their own path: "/Outpath/Output/",
    file_meta = "usr",
...
All the output will be stored in the `Output` directory of the working directory or the path `/Outpath/Output/` that user specified.
...
## Using the Example Datasets in the MapfxData Package
