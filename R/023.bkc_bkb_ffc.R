...
function(
    ...
    para.df <- data.frame(Batch = unique(metadata.cell$Batch), mu = NA, sig = NA, alpha = NA)
    for(w in seq_len(length(unique(metadata.cell$Batch)))){
    dat.here <- sel.bkb.raw[which(metadata.cell$Batch == para.df$Batch[w]), m]
    ...
    #concatenating calib values for each batch
